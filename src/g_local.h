...
constexpr const char *GAMEVERSION = "zaereo";
...
// Zaero stores campaign/mirror filtering in a second, independent flag word.
// Do not merge these values into spawnflags_t: the bit namespaces overlap with
// ordinary per-entity flags and shipped maps contain both fields.
enum zaero_spawnflags2_t : uint32_t
{
...
MAKE_ENUM_BITFLAGS(zaero_spawnflags2_t);

constexpr bool ZaeroHasSpawnflag2(zaero_spawnflags2_t value, zaero_spawnflags2_t flag)
{
	return (value & flag) != ZAERO_SPAWNFLAG2_NONE;
}
...
enum movetype_t {
	...
	// ZAERO: append these so the native Rerelease movement IDs remain stable.
	MOVETYPE_BOUNCEFLY, // no gravity; reflects without losing speed
	MOVETYPE_FALLFLOAT, // slides down steep slopes and floats in water
	MOVETYPE_RIDE       // step physics plus up to two attached child entities
};

static_assert(MOVETYPE_NEWTOSS == 11, "Rerelease movetype save IDs changed");
static_assert(MOVETYPE_BOUNCEFLY == 12 && MOVETYPE_FALLFLOAT == 13 && MOVETYPE_RIDE == 14,
	"Zaero movetype save IDs changed");
...
enum item_flags_t : uint32_t
{
	...
	// ZAERO's legacy hideFlags were three independent contracts. Keep them
	// separate so Sniper/A2K can remain visible in inventory while staying out
	// of automatic weapon selection, and Push can preserve HIDE_DONT_KEEP.
	IF_NO_INVENTORY = bit_v<14>,
	IF_NO_WEAPON_SELECTION = bit_v<15>,
	IF_TRANSIENT_WEAPON = bit_v<16>,

	// Temporary development marker for an exact registered item whose weapon
	// callback is deliberately non-functional. Remove it with the placeholder.
	IF_PENDING_IMPLEMENTATION = bit_v<17>,
...
enum item_id_t : int32_t {
	...
	// ZAERO. Appended so upstream item IDs retain their established values;
	// JSON saves resolve items by stable classname rather than these numbers.
	IT_WEAPON_PUSH,
	IT_WEAPON_FLAREGUN,
	IT_AMMO_IRED,
...
static_assert(IT_TOTAL <= MAX_ITEMS, "Zaero item IDs overflow the engine inventory protocol");
...
enum mod_id_t : uint8_t
{
	...
	// ZAERO. Appended so all native and mission-pack means-of-death keep their
	// established numeric IDs.
	MOD_ZAERO_FLARE,
...
// Zaero's func_rotating uses a separate four-state angular velocity machine.
// Do not overload moveinfo.state: Rerelease bots interpret that field as the
// positional mover states above.
enum class zaero_rotating_state_t : uint8_t
{
	stopped,
	accelerating,
	top_speed,
...
struct monsterinfo_t
{
	...
	// ZAERO Flare blindness uses the original 10 Hz reference-tick values.
...
// Legacy Zaero reserves the low 0x10 bit for monsters that must not contribute
// to the level kill total.  This is distinct from Rerelease's high-bit
// SPAWNFLAG_MONSTER_DEAD extension.
constexpr spawnflags_t SPAWNFLAG_ZAERO_MONSTER_NO_COUNT = 16_spawnflag;
...
void Weapon_Generic(edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST,
					...
					void (*fire)(edict_t *ent), bool play_powerup_sound = true);
...
struct gclient_t
{
	...
	// ZAERO transient utilities and explicit pending callbacks return to the
	// previous weapon without replacing the ordinary weaplast history.
	gitem_t *zaero_transient_lastweapon;
	float zaero_flare_flash_ticks;
	float zaero_flare_flash_base;
	gtime_t zaero_sonic_charge_start;
...
struct edict_t
{
	...
	const char *model;
	// ZAERO: linked models used by multipart map objects.
	const char *model2;
	const char *model3;
	const char *model4;
	...
	zaero_spawnflags2_t spawnflags2;
	...
	float  aspeed; // ZAERO: angular acceleration/speed for custom movers
	zaero_rotating_state_t zaero_rotating_state;
	vec3_t movedir;
	...
	float random;
	// Semicolon func_timer targets are fixed-width, level-owned slots. This
	// mirrors the legacy lifetime without its unchecked copies and gives a
	...
	vec3_t move_origin;
	vec3_t move_angles;
	vec3_t mangle; // ZAERO: camera orientation, intentionally separate from angles
	...
	// ZAERO map/gameplay state. These live in the game-private portion of the
	// edict and are included in the JSON save schema in g_save.cpp.
	int32_t active;
	const char *mteam;

	// MOVETYPE_RIDE attachments. The spawn counts prevent a freed rider slot
	// from accidentally attaching a subsequently allocated entity.
	static constexpr size_t ZAERO_MAX_RIDERS = 2;
	std::array<edict_t *, ZAERO_MAX_RIDERS> ride_with;
	std::array<vec3_t, ZAERO_MAX_RIDERS> ride_with_offset;
	std::array<int32_t, ZAERO_MAX_RIDERS> ride_with_spawn_count;
	// NOTE: if adding new elements, make sure to add them
	// in g_save.cpp too!
};
