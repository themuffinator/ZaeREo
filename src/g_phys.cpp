...
bool G_SetZaeroRider(edict_t *carrier, size_t slot, edict_t *rider)
{
	if (!carrier || !carrier->inuse || !rider || !rider->inuse || carrier == rider ||
		slot >= edict_t::ZAERO_MAX_RIDERS)
		return false;

	carrier->ride_with[slot] = rider;
	carrier->ride_with_offset[slot] = rider->s.origin - carrier->s.origin;
	carrier->ride_with_spawn_count[slot] = rider->spawn_count;
	return true;
}

void G_ClearZaeroRiders(edict_t *carrier)
{
	if (!carrier)
		return;

	carrier->ride_with = {};
	carrier->ride_with_offset = {};
	carrier->ride_with_spawn_count = {};
}

static void SV_AdjustZaeroRiders(edict_t *carrier)
{
	for (size_t slot = 0; slot < edict_t::ZAERO_MAX_RIDERS; ++slot)
	{
		edict_t *rider = carrier->ride_with[slot];

		// Edict storage is stable, but a freed slot can be reused. Keep the saved
		// generation alongside the pointer so a replacement entity is never moved.
		if (!rider)
			continue;
		if (!rider->inuse || rider->spawn_count != carrier->ride_with_spawn_count[slot])
		{
			carrier->ride_with[slot] = nullptr;
			carrier->ride_with_offset[slot] = {};
			carrier->ride_with_spawn_count[slot] = 0;
			continue;
		}

		rider->s.origin = carrier->s.origin + carrier->ride_with_offset[slot];
		gi.linkentity(rider);
	}
}
...
retry:
	...
	{
		// Zaero's floor autocannon base carries two MOVETYPE_NONE model pieces.
		// Defer them until every part of the pusher team succeeds so a later
		// blocked team slave cannot leave children at an uncommitted position.
		for (pushed_t *moved = pushed; moved < pushed_p; moved++)
			if (moved->ent->inuse && moved->ent->movetype == MOVETYPE_RIDE)
				SV_AdjustZaeroRiders(moved->ent);
...
void SV_Physics_Toss(edict_t *ent)
{
	...
	if (ent->movetype != MOVETYPE_FLY &&
		...
		// ZAERO: bounce-fly projectiles retain their speed and do not fall.
	...
	while (time_left)
	{
		...
		const float bouncefly_speed = ent->movetype == MOVETYPE_BOUNCEFLY ? ent->velocity.length() : 0.f;
		...
		{
			// RAFAEL
			if (ent->movetype == MOVETYPE_WALLBOUNCE || ent->movetype == MOVETYPE_BOUNCEFLY)
				backoff = 2.0f;
			// RAFAEL
			else
				backoff = 1.6f;
		...
		{
			// ClipVelocity's overbounce can change magnitude. Zaero deliberately
			// restores the incident speed after choosing the reflected direction.
			if (ent->velocity.normalize() > 0.f)
				ent->velocity *= bouncefly_speed;
		}
		else if (ent->movetype == MOVETYPE_WALLBOUNCE)
...
// Zaero's original physics ran at ten ticks per second. These values are not
// another simulation clock: they translate its per-frame damping and threshold
// constants into frame-rate-independent quantities for the Rerelease tick rate.
static constexpr float ZAERO_REFERENCE_FRAME_SECONDS = 0.1f;
static constexpr float ZAERO_WATER_DAMPING_PER_REFERENCE_FRAME = 0.7f;
static constexpr float ZAERO_WATER_DENSITY = 500.f;
static constexpr float ZAERO_MIN_BUOYANCY_ACCELERATION = 4.f / ZAERO_REFERENCE_FRAME_SECONDS;

static float SV_ZaeroSubmergedFraction(const edict_t *ent)
{
	float fraction = 1.f;

	// Preserve the legacy sampling path. Zaero's FALLFLOAT objects have mins.z
	// zero, so origin + mins + maxs*t traverses their authored vertical extent.
	for (int sample = 0; sample <= 20; ++sample)
	{
		const float t = sample * 0.05f;
		const vec3_t point = ent->s.origin + ent->mins + (ent->maxs * t);
	...
	return fraction < 0.05f ? 0.f : clamp(fraction, 0.f, 1.f);
}

static void SV_ZaeroUpdateWaterTransition(edict_t *ent, const vec3_t &old_origin)
{
	const bool was_in_water = (ent->watertype & MASK_WATER) != 0;
	ent->watertype = gi.pointcontents(ent->s.origin);
	const bool is_in_water = (ent->watertype & MASK_WATER) != 0;
	ent->waterlevel = is_in_water ? WATER_FEET : WATER_NONE;
...
static void SV_Physics_FallFloat(edict_t *ent)
{
	SV_CheckVelocity(ent);

	// This is intentionally inverted: the legacy FALLFLOAT landing-sound gate
	// recorded `groundentity == NULL`. Preserve the harmless observable quirk
	// until shipped-map captures justify a separately recorded FIX (Q-021).
	const bool legacy_was_on_ground = ent->groundentity == nullptr;
	vec3_t gravity_direction = ent->gravityVector.normalized();
	if (!gravity_direction)
		gravity_direction = { 0.f, 0.f, -1.f };

	const float frame_seconds = gi.frame_time_s;
	const float gravity_acceleration = ent->gravity * level.gravity;
	const bool hit_sound = ent->velocity.dot(gravity_direction) > gravity_acceleration * 0.1f;

	if (ent->waterlevel == WATER_NONE)
	{
		const vec3_t end = ent->s.origin + (gravity_direction * 0.25f);
		const trace_t tr = gi.trace(ent->s.origin, ent->mins, ent->maxs, end, ent, G_GetClipMask(ent));
		const float support = tr.plane.normal.dot(-gravity_direction);

		if (tr.fraction < 1.f && support > 0.7f)
		{
		...
		{
			// The legacy code treats this as a stable slope velocity rather than
			// accumulated acceleration. Retain its 10 Hz speed at any target tick.
			const float reference_gravity_step = gravity_acceleration * ZAERO_REFERENCE_FRAME_SECONDS;
			ent->velocity = (gravity_direction * reference_gravity_step) +
				(tr.plane.normal * reference_gravity_step);
			ent->groundentity = nullptr;
		}
		else
		{
			ent->velocity += gravity_direction * (gravity_acceleration * frame_seconds);
			ent->groundentity = nullptr;
		}
	}
	else
	{
		const vec3_t volume = ent->size * (1.f / 32.f);
		const float displaced_mass = ZAERO_WATER_DENSITY * volume.x * volume.y * volume.z;
		const float damping = powf(ZAERO_WATER_DAMPING_PER_REFERENCE_FRAME,
			frame_seconds / ZAERO_REFERENCE_FRAME_SECONDS);
		ent->velocity *= damping;

		if (std::isfinite(displaced_mass) && displaced_mass > 0.f)
		{
			// All shipped FALLFLOAT spawns provide mass 400. Clamp malformed or
			// source-only zero/negative masses without changing valid map behavior.
			const float object_mass = max(1.f, static_cast<float>(ent->mass));
			const float submerged_mass = SV_ZaeroSubmergedFraction(ent) * displaced_mass;
			const float ratio = (submerged_mass - object_mass) / displaced_mass;
			const vec3_t buoyancy_acceleration = -gravity_direction * (gravity_acceleration * ratio);

			if (buoyancy_acceleration.length() > ZAERO_MIN_BUOYANCY_ACCELERATION)
				ent->velocity += buoyancy_acceleration * frame_seconds;
		}
		else
		{
			// The legacy calculation divided by zero for degenerate bounds. A
			// malformed zero-volume object now sinks normally instead of becoming NaN.
			ent->velocity += gravity_direction * (gravity_acceleration * frame_seconds);
		}
	}
	...
	if (ent->velocity)
	{
		...
		SV_FlyMove(ent, frame_seconds, G_GetClipMask(ent));
		G_TouchProjectiles(ent, old_origin);
		if (!ent->inuse)
			return;
		...
		if (ent->groundentity && !legacy_was_on_ground && hit_sound)
			ent->s.event = EV_FOOTSTEP;

		SV_ZaeroUpdateWaterTransition(ent, old_origin);
	}
	else
	{
		gi.linkentity(ent);
...
static void SV_Physics_Ride(edict_t *ent)
{
	SV_Physics_Step(ent);
	if (ent->inuse)
		SV_AdjustZaeroRiders(ent);
}
