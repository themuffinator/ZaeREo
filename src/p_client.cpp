...
void ClientObituary(edict_t *self, edict_t *inflictor, edict_t *attacker, mod_t mod)
{
	...
	if (attacker && attacker->client)
	{
		...
		case MOD_GRAPPLE:
			...
			break;
			// ZOID
		case MOD_ZAERO_FLARE:
			base = "{} didn't see {}'s flare\n";
			break;
		case MOD_ZAERO_GL_POLYBLEND:
			base = "{} turned off gl_polyblend and was damaged by {}'s flare\n";
			break;
		case MOD_ZAERO_SONIC_CANNON:
			base = "{} got microwaved by {}\n";
			break;
		case MOD_ZAERO_TRIPBOMB:
			base = "{} tripped over {}'s trip bomb\n";
			break;
...
void InitClientPersistant(edict_t *ent, gclient_t *client, init_client_persistant_mode_t mode)
{
		...
		(!G_TeamplayEnabled() && !client->resp.spectator))
	{
		...
		if (!taken_loadout)
		{
			...
			client->pers.max_ammo[AMMO_FLARES] = 30;
			client->pers.max_ammo[AMMO_IRED] = 30;
			client->pers.max_ammo[AMMO_A2K] = 1;
			client->pers.max_ammo[AMMO_EMPNUKE] = 50;
			client->pers.max_ammo[AMMO_PLASMASHIELD] = 20;
		...
		// ZAERO's hidden Push utility is always owned. Campaign/co-op starts
		// additionally retain the Blaster and begin with a Flare Gun and three
		// flares; deathmatch starts only add Push, matching the supplied DLL.
		client->pers.inventory[IT_WEAPON_PUSH] = 1;
		if (!deathmatch->integer)
		{
			client->pers.inventory[IT_WEAPON_BLASTER] = 1;
			client->pers.inventory[IT_WEAPON_FLAREGUN] = 1;
			client->pers.inventory[IT_AMMO_FLARES] = max(3, client->pers.inventory[IT_AMMO_FLARES]);
		}
...
void P_AssignClientSkinnum(edict_t *ent)
{
	...
	// Zaero weapons do not all provide a Rerelease vwep model.  Keep the
	// packed index at the native "no visible weapon" value instead of
	// underflowing the offset calculation for those legacy definitions.
	if (ent->client->pers.weapon && ent->client->pers.weapon->vwep_index)
