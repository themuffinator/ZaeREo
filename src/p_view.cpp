...
void SV_CalcGunOffset(edict_t *ent)
{
	...
	if (ent->client->pers.weapon && 
		...
		&& !Zaero_SniperSuppressGunOffset(ent)
		&& !SkipViewModifiers())
	{
		// ROGUE
...
void SV_CalcBlend(edict_t *ent)
{
	...
	// ZAERO Flare blindness is client-private presentation state. Blend it into
	// the Rerelease screen channel so split-screen clients cannot leak state.
	Zaero_AddFlareBlend(ent);
...
void G_SetClientSound(edict_t *ent)
{
	...
	else if (ent->client->pers.weapon)
	{
		...
		else if (ent->client->pers.weapon->id == IT_WEAPON_SONICCANNON)
