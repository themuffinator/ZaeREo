...
static const std::initializer_list<spawn_t> spawns = {
	...
	// ZAERO mapper-facing world entities
...
static const std::initializer_list<field_t> entity_fields = {
	...
	FIELD_AUTO(mangle), // ZAERO remote-camera orientation
	{ "mirrortarget" }, // ZAERO mirror metadata; legacy runtime no-op
	{ "mirrorlevelsave" }, // ZAERO mirror metadata; legacy runtime no-op
...
inline bool G_InhibitEntity(edict_t *ent)
{
	...
	if (deathmatch->integer)
		return ent->spawnflags.has(SPAWNFLAG_NOT_DEATHMATCH) ||
			ZaeroHasSpawnflag2(ent->spawnflags2, ZAERO_SPAWNFLAG2_NOT_SINGLE);

	// Zaero's independent campaign flags are evaluated before the stock
	// Rerelease skill/co-op flags. MIRRORLEVEL is retained but intentionally
	// does not inhibit entities; the shipped legacy mirror loader is dormant.
	if (coop->integer && ZaeroHasSpawnflag2(ent->spawnflags2, ZAERO_SPAWNFLAG2_NOT_COOP))
		return true;
	if (!coop->integer && ZaeroHasSpawnflag2(ent->spawnflags2, ZAERO_SPAWNFLAG2_NOT_SINGLE))
		return true;
