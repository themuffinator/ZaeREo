...
// Zaero's mapper-facing "active" key is a separate bit namespace from
// spawnflags.  Bit 1 opts a sliding door into eligibility toggling and bit 2
// records whether its automatic touch trigger is currently enabled.
constexpr int32_t ZAERO_DOOR_ACTIVE_TOGGLE = 1;
constexpr int32_t ZAERO_DOOR_ACTIVE_ON = 2;
...
On Zaero maps, accel/decel are degrees per second squared and are active without
the Rerelease ACCEL flag; speed remains degrees per second.
...
void rotating_touch(edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self);

// Zaero expresses angular acceleration and deceleration in degrees per second
// squared. Its original 10 Hz code multiplied the rate by 0.1; use the native
// frame duration so the same curve remains correct at Rerelease's 40 Hz tick.
THINK(zaero_rotating_think) (edict_t *self) -> void
{
	self->nextthink = level.time + FRAME_TIME_S;

	if (self->zaero_rotating_state == zaero_rotating_state_t::decelerating)
	{
		...
		{
			// Preserve the legacy boundary behavior: a final decrement may cross
			// zero for one tick before the next think clamps it.
			self->moveinfo.current_speed -= self->decel * gi.frame_time_s;
		}
	}
	else if (self->zaero_rotating_state == zaero_rotating_state_t::accelerating)
	{
		if (self->moveinfo.current_speed >= self->speed)
		{
			self->moveinfo.current_speed = self->speed;
			self->zaero_rotating_state = zaero_rotating_state_t::top_speed;
			self->think = nullptr;
			self->nextthink = 0_ms;
		}
		else
		{
			self->moveinfo.current_speed += self->accel * gi.frame_time_s;
		}
	}

	self->avelocity = self->movedir * self->moveinfo.current_speed;
}

static void zaero_rotating_use(edict_t *self)
{
	if (self->zaero_rotating_state == zaero_rotating_state_t::top_speed ||
		self->zaero_rotating_state == zaero_rotating_state_t::accelerating)
	{
		if (self->decel <= 0)
		{
			self->s.sound = 0;
		...
		{
			self->think = zaero_rotating_think;
			self->nextthink = level.time + FRAME_TIME_S;
			self->zaero_rotating_state = zaero_rotating_state_t::decelerating;
		}
		return;
	}
...
void door_touch(edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self);

// Perform ordinary door motion without interpreting Zaero's mapper-facing
// eligibility bits.  The original Touch_DoorTrigger called door_openclose
// directly: an enabled automatic trigger must open the door, not immediately
// toggle itself back off through door_use.
...
USE(door_use) (edict_t *self, edict_t *other, edict_t *activator) -> void
{
	if (level.zaero_mapper_contract && (self->active & ZAERO_DOOR_ACTIVE_TOGGLE))
	{
		...
		// guard: the supplied use callback can be invoked on each targeted team
		// brush, and that observable touch-state quirk is part of parity.
		for (edict_t *ent = self; ent; ent = ent->teamchain)
		{
			if (ent->active & ZAERO_DOOR_ACTIVE_ON)
			{
				ent->touch = door_touch;
				ent->active &= ~ZAERO_DOOR_ACTIVE_ON;
			}
			else
			{
				ent->touch = nullptr;
...
void SP_func_door(edict_t *ent)
{
	...
	if (level.zaero_mapper_contract)
	{
		...
		// accepts touches.  This ordering is the supplied Zaero spawn contract,
		// including START_OPEN doors; stock maps retain the Rerelease path below.
		if (!(ent->active & ZAERO_DOOR_ACTIVE_TOGGLE))
			ent->active = 0;

		if ((ent->health || ent->targetname) && !(ent->active & ZAERO_DOOR_ACTIVE_TOGGLE))
...
On Zaero maps, a semicolon-delimited target chooses one of at most 16 target
names randomly per firing. A target without semicolons keeps native behavior.
...
constexpr size_t ZAERO_TIMER_MAX_TARGETS = 16;
...
static bool func_timer_parse_zaero_targets(edict_t *self)
{
	...
	const std::string_view target_list(self->target);
	size_t cursor = 0;

	while (cursor <= target_list.size())
	{
		const size_t separator = target_list.find(';', cursor);
		const size_t end = separator == std::string_view::npos ? target_list.size() : separator;
		const std::string_view candidate = target_list.substr(cursor, end - cursor);

		// strtok in the legacy implementation skipped empty fields.
		if (!candidate.empty())
		{
			if (target_count == parsed_targets.size())
			{
				gi.Com_PrintFmt("{}: func_timer has more than {} random targets\n", *self, ZAERO_TIMER_MAX_TARGETS);
				return false;
			}
			if (candidate.size() >= MAX_QPATH)
			{
				gi.Com_PrintFmt("{}: func_timer random target is longer than {} bytes\n", *self, MAX_QPATH - 1);
				return false;
			}
			parsed_targets[target_count++] = candidate;
		}
	...
	if (!target_count)
	{
		gi.Com_PrintFmt("{}: func_timer random target list is empty\n", *self);
		return false;
	}

	self->zaero_timer_targets = make_savable_memory<uint8_t, TAG_LEVEL>(target_count * MAX_QPATH);
	memset(self->zaero_timer_targets.ptr, 0, self->zaero_timer_targets.count);
	for (size_t i = 0; i < target_count; ++i)
		memcpy(self->zaero_timer_targets.ptr + (i * MAX_QPATH), parsed_targets[i].data(), parsed_targets[i].size());

	self->zaero_timer_target_count = static_cast<uint8_t>(target_count);
	self->target = nullptr;
	return true;
}
...
{
	const int32_t spawn_count = self->spawn_count;
	const bool random_target_list = self->zaero_timer_target_count && self->zaero_timer_targets;

	if (random_target_list)
	{
		const size_t choice = irandom(self->zaero_timer_target_count);
		self->target = reinterpret_cast<const char *>(self->zaero_timer_targets.ptr + (choice * MAX_QPATH));
	}

	G_UseTargets(self, self->activator);
	if (!self->inuse || self->spawn_count != spawn_count)
		return;
	if (random_target_list)
		self->target = nullptr;
...
void SP_func_timer(edict_t *self)
{
	if (!self->wait)
		self->wait = 1.0;
	if (!func_timer_parse_zaero_targets(self))
	{
		G_FreeEdict(self);
		return;
	}
