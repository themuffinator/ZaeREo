...
TOUCH(trigger_push_touch) (edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self) -> void
{
	if (level.is_zaero && self->spawnflags.has(SPAWNFLAG_PUSH_ZAERO_START_OFF))
	{
		...
		// projectiles and monsters touching the volume.
		if (other->client && self->message && self->touch_debounce_time < level.time)
		{
...
Zaero maps use bit 2 as START_OFF and bit 4 as NO_SOUND. Other maps retain the
Rerelease PUSH_PLUS/PUSH_SILENT/START_OFF meanings shown in the flag list.
*/
void SP_trigger_push(edict_t *self)
{
	...
	else if (self->targetname) // toggleable
	{
		self->use = trigger_push_use;
