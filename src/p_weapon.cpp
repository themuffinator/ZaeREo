...
void Use_Weapon(edict_t *ent, gitem_t *item)
{
	...
	{
		if ((wanted->flags & (IF_TRANSIENT_WEAPON | IF_PENDING_IMPLEMENTATION)) &&
			wanted != ent->client->pers.weapon)
...
void Weapon_Generic(edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, const int *pause_frames, const int *fire_frames, void (*fire)(edict_t *ent), bool play_powerup_sound)
{
...
void Weapon_RocketLauncher_Fire(edict_t *ent)
{
	...
	if (Zaero_EMPNukeCheck(ent, start))
	{
...
bool Blaster_Fire(edict_t *ent, const vec3_t &g_offset, int damage, bool hyper, effects_t effect)
{
	if (is_quad)
		damage *= damage_multiplier;
	...
	if (request_firing)
	{
		if (ent->client->ps.gunframe >= 6 && ent->client->ps.gunframe <= 11)
		{
			...
			if (deathmatch->integer)
				...
				damage = 20;
			const bool fired = Blaster_Fire(ent, offset, damage, true, (ent->client->ps.gunframe % 4) ? EF_NONE : EF_HYPERBLASTER);
			Weapon_PowerupSound(ent);
...
void weapon_bfg_fire(edict_t *ent)
{
	...
	vec3_t start, dir;
	P_ProjectSource(ent, ent->client->v_angle, { 8, 8, -8 }, start, dir);
	P_AddWeaponKick(ent, ent->client->v_forward * -2, vec3_origin);

	// The supplied weapon calls this function at frames 9 and 17. A blocked
	// wind-up plays one misfire sound and latches the second callback off too.
	if (ent->client->ps.gunframe == 9)
		ent->client->zaero_bfg_emp_misfire = false;
	if (!ent->client->zaero_bfg_emp_misfire && Zaero_EMPNukeCheck(ent, start))
	{
		ent->client->zaero_bfg_emp_misfire = true;
		Zaero_PlayEMPMisfire(ent);
	}
	if (ent->client->zaero_bfg_emp_misfire)
		return;
