...
bool visible(edict_t *self, edict_t *other, bool through_glass)
{
    ...
    // ZAERO monsters cannot acquire or maintain visual contact while Flare
    // blindness remains. Player and non-monster visibility is unchanged.
    if ((self->svflags & SVF_MONSTER) && self->monsterinfo.zaero_flare_flash_ticks > 0.0f)
        return false;
...
void ai_run(edict_t *self, float dist)
{
    ...
    if (self->monsterinfo.zaero_flare_flash_ticks > 0.0f)
    {
        Zaero_MonsterMoveAwayFromFlare(self, dist);
        return;
    }
