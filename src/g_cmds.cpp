...
void ValidateSelectedItem(edict_t *ent)
{
	...
	if (cl->pers.inventory[cl->pers.selected_item] &&
		!(itemlist[cl->pers.selected_item].flags & IF_NO_INVENTORY))
		return; // valid
...
void Cmd_Inven_f(edict_t *ent)
{
	...
	for (i = 0; i < IT_TOTAL; i++)
		gi.WriteShort((itemlist[i].flags & IF_NO_INVENTORY) ? 0 : cl->pers.inventory[i]);
	for (; i < MAX_ITEMS; i++)
		gi.WriteShort(0);
	gi.unicast(ent, true);
}
...
void Cmd_WeapPrev_f(edict_t *ent)
{
	...
	for (i = static_cast<item_id_t>(IT_NULL + 1); i <= IT_TOTAL; i = static_cast<item_id_t>(i + 1))
	{
		...
		if (!it->use)
			continue;
		if (it->flags & IF_NO_WEAPON_SELECTION)
			continue;
		if (!(it->flags & IF_WEAPON))
			continue;
		it->use(ent, it);
