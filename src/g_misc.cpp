...
/*QUAKED misc_viper (1 .5 0) (-16 -16 0) (16 16 32) SMOKE SOLID CUSTOM_BOUNDS
...
Zaero maps additionally use SMOKE for the legacy rocket trail, SOLID for a
bounding box, and CUSTOM_BOUNDS to retain mapper-authored mins/maxs.  model,
model2, model3, and model4 select the multipart ship presentation.  These bits
remain native Rerelease train bits outside a detected Zaero map.
*/

constexpr spawnflags_t SPAWNFLAG_VIPER_ZAERO_SMOKE = 1_spawnflag;
constexpr spawnflags_t SPAWNFLAG_VIPER_ZAERO_SOLID = 2_spawnflag;
constexpr spawnflags_t SPAWNFLAG_VIPER_ZAERO_CUSTOM_BOUNDS = 4_spawnflag;
...
void SP_misc_viper(edict_t *ent)
{
	...
	if (level.is_zaero)
	{
		...
		// TRAIN_START_ON.
		if (ent->spawnflags.has(SPAWNFLAG_VIPER_ZAERO_SMOKE))
		{
			ent->s.effects |= EF_ROCKET;
			ent->spawnflags &= ~SPAWNFLAG_VIPER_ZAERO_SMOKE;
		}

		ent->solid = ent->spawnflags.has(SPAWNFLAG_VIPER_ZAERO_SOLID) ? SOLID_BBOX : SOLID_NOT;
		ent->s.modelindex = gi.modelindex(ent->model ? ent->model : "models/ships/viper/tris.md2");
		...
		if (!ent->spawnflags.has(SPAWNFLAG_VIPER_ZAERO_CUSTOM_BOUNDS))
		{
			ent->mins = { -16, -16, 0 };
			ent->maxs = { 16, 16, 32 };
		}
