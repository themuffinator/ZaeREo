...
/*
=================
Zaero_MonsterKillBox

Zaero deliberately protects a live player who occupies a triggered monster's
spawn volume.  Non-player blockers are telefragged normally; if a live player
still overlaps, the newly spawned monster telefrags itself.  Keep this helper
separate from native KillBox so stock/Rerelease spawn collision policy is not
changed.
=================
*/
bool Zaero_MonsterKillBox(edict_t *ent)
{
	...
	static edict_t *touch[MAX_EDICTS];
	const int32_t num = gi.BoxEdicts(ent->absmin, ent->absmax, touch, MAX_EDICTS,
		AREA_SOLID, KillBox_BoxFilter, nullptr);
	bool live_player_overlap = false;

	for (int32_t i = 0; i < num; i++)
	{
		edict_t *hit = touch[i];
		if (hit == ent || !hit->inuse || !hit->takedamage || !hit->solid ||
			hit->solid == SOLID_TRIGGER || hit->solid == SOLID_BSP)
			continue;

		if (hit->client && hit->health > 0)
		{
			live_player_overlap = true;
			continue;
		}

		T_Damage(hit, ent, ent, vec3_origin, ent->s.origin, vec3_origin,
			100000, 0, DAMAGE_NO_PROTECTION, MOD_TELEFRAG);
	}

	if (live_player_overlap && ent->inuse && ent->health > 0)
		T_Damage(ent, ent, ent, vec3_origin, ent->s.origin, vec3_origin,
			100000, 0, DAMAGE_NO_PROTECTION, MOD_TELEFRAG);

	return !live_player_overlap;
}
