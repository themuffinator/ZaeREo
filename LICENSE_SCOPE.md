...
| --- | --- | --- |
| Quake II Rerelease substrate | The supplied baseline contains GNU GPL version 2 notices and the repository includes the GPL-2.0 text. The precise official source origin is not yet pinned in the baseline record. | **Unknown.** The GPL evidence applies to code placed under it by its holders; it does not by itself establish the provenance of the imported snapshot or license unrelated additions. |
...
| fmt | The pinned build declares fmt. `THIRD_PARTY_NOTICES.md` records an observed MIT license and required exact notice harvest. | **Candidate dependency only.** A binary release remains blocked until the exact pinned license file, bytes, SBOM, and notices are tied to that candidate. |
| JsonCpp | The pinned build declares JsonCpp. `THIRD_PARTY_NOTICES.md` records its observed public-domain/MIT alternatives and required exact notice harvest. | **Candidate dependency only.** The applicable exact notice and candidate byte coverage remain required. |
